          ...
          tabs: [
            const Tab(
              child: Row(
                ...
                children: [Icon(Icons.map_outlined, size: 15), SizedBox(width: 5), Text('포인트 지도')],
              ),
            ),
            Tab(
              child: Row(
      ...
      body: TabBarView(
        ...
        children: [
          _MapTab(
            ...
            onSelectMarker: (m) {
              setState(() => _selectedMarker = (_selectedMarker?.id == m?.id) ? null : m);
            },
            onDeleteUserSpot: (m) {
              if (m.record != null) _confirmDeleteRecord(m.record!);
            },
  ...
  void initState() {
    ...
    _mapEventSub = _mapController.mapEventStream.listen((e) {
      ...
      try {
        ...
        if ((rot - _currentRotation).abs() > 0.5) {
          setState(() => _currentRotation = rot);
        }
      } catch (_) {}
    });
  }
    ...
    try {
      ...
      if (perm == LocationPermission.deniedForever || perm == LocationPermission.denied) {
        if (mounted) {
          ...
          _moveToLatLng(kBusanCenter, zoom: 12);
        }
        return;
      }
      ...
      if (!serviceEnabled) {
        if (mounted) {
          _showSnack('GPS가 꺼져있습니다. 설정에서 켜주세요.', isError: true);
          _moveToLatLng(kBusanCenter, zoom: 12);
        }
        return;
      }
      ...
      if (mounted) {
        _showSnack('현재 위치를 가져올 수 없어요. 부산으로 이동합니다.', isError: true);
        _moveToLatLng(kBusanCenter, zoom: 12);
      }
    } finally {
  ...
  Marker _buildMarker(MapMarker m) {
    ...
    return Marker(
      ...
      child: GestureDetector(
        onTap: () {
          widget.onSelectMarker(m);
          _moveToLatLng(m.latLng);
        },
        child: Stack(
          ...
          children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(
                ...
                boxShadow: [
                  BoxShadow(
                    color: (m.isUser ? kOrange : kPrimary).withOpacity(0.4),
                    blurRadius: 8,
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final mapHeight = fullHeight * 0.5;
                ...
                child: ClipRect(
                  child: FlutterMap(
                    ...
                    options: MapOptions(
                      ...
                      onTap: (_, __) {
                        if (widget.selectedMarker != null) widget.onSelectMarker(null);
                      },
                    ),
                    children: [
                      TileLayer(
                  ...
                  children: [
                    if (_currentRotation.abs() > 1)
                      Padding(
                        ...
                        child: _mapCtrlBtn(Icons.explore, _resetRotation, color: kNavy),
                      ),
                    ...
                    const SizedBox(height: 8),
                    _mapCtrlBtn(
                      _gpsLoading ? Icons.hourglass_top : Icons.my_location_rounded,
                          ...
                          child: Center(
                            child: Column(
                              ...
                              children: [
                                ...
                                const SizedBox(height: 8),
                                Text(
                                  _filter == '사용자' ? '아직 추가한 포인트가 없어요' : '$_filter 난이도 포인트가 없어요',
                              ...
                              (ctx, i) {
                                ...
                                return Padding(
                                  ...
                                  child: _SpotListCard(
                                    spot: spot,
                                    isSelected: isSel,
                                    diffColor: _diffColor(spot.diff),
                                    diffBg: _diffBg(spot.diff),
                                    onTap: () {
                                      widget.onSelectMarker(spot);
                                      _moveToLatLng(spot.latLng, zoom: 15);
                                    },
                                  ),
                                );
                              },
...
class _InfoPopup extends StatelessWidget {
  ...
  const _InfoPopup({required this.marker, required this.onClose, required this.onDelete});
                        ...
                        children: [
                          Expanded(
                            child: r != null
                                ? Row(
                                    children: [
                                      const Icon(Icons.access_time, size: 10, color: kSub),
                            ...
                            GestureDetector(
                              ...
                              child: Container(
                                ...
                                child: const Row(
                                  mainAxisSize: MainAxisSize.min,
                                  children: [
                                    Icon(Icons.delete_outline, size: 12, color: kRed),
                                    SizedBox(width: 3),
                                    Text(
                                      '삭제',
                                      style: TextStyle(fontSize: 11, color: kRed, fontWeight: FontWeight.bold),
                                    ),
                                  ],
                                ),
                              ),
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(16),
          border: Border.all(
            color: isSelected ? (spot.isUser ? kOrange : kPrimary) : kBorder,
            width: isSelected ? 1.5 : 1,
          ),
          boxShadow: isSelected
              ? [
                  BoxShadow(
                    color: (spot.isUser ? kOrange : kPrimary).withOpacity(0.2),
                    blurRadius: 15,
        ...
        child: Row(children: [
          Container(
            ...
            decoration: BoxDecoration(
              color: isSelected
                  ? (spot.isUser ? kOrange.withOpacity(0.2) : kPrimary.withOpacity(0.15))
                  : Colors.grey[100],
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Row(children: [
                  ...
                  if (spot.isUser)
                    Container(
                      padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                      decoration: BoxDecoration(
                        color: isSelected ? kOrange.withOpacity(0.3) : kOrange.withOpacity(0.12),
                        borderRadius: BorderRadius.circular(5),
                      ),
                      child: const Text(
                        '📍 내 포인트',
                        style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: kOrange),
                      ),
                    )
                  else
                    Container(
                      padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                      decoration: BoxDecoration(
                        color: isSelected ? diffColor.withOpacity(0.25) : diffBg,
                        borderRadius: BorderRadius.circular(6),
                      ),
                      child: Text(
                        spot.diff,
                ...
                Wrap(
                  spacing: 6,
                  children: spot.species
                      .map((s) => Text(
                            '#$s',
                            style: TextStyle(
                              fontSize: 13,
                              fontWeight: FontWeight.w500,
                              color: isSelected ? Colors.white70 : kSub,
                            ),
                          ))
                      .toList(),
                ),
                ...
                Text(
                  ...
                  style: TextStyle(
                    fontSize: 11,
                    color: isSelected ? Colors.white38 : Colors.grey[400],
                  ),
                ),
              ],
            ),
...
class _JournalTab extends StatelessWidget {
  ...
  const _JournalTab({required this.records, required this.onDelete, required this.onAddRecord});
            ...
            SliverToBoxAdapter(
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  boxShadow: [
                    BoxShadow(color: kNavy.withOpacity(0.28), blurRadius: 14, offset: const Offset(0, 4)),
                  ],
                ),
                child: Row(
      ...
      child: Stack(children: [
        ...
        GestureDetector(
          onHorizontalDragUpdate: (d) {
            setState(() => _dragX = (_dragX + d.delta.dx).clamp(-_threshold, 0));
          },
          onHorizontalDragEnd: (d) {
            setState(() => _dragX = _dragX < -_threshold / 2 ? -_threshold : 0);
          },
              ...
              child: Column(children: [
                          ...
                          if (hasLocation && r.locationName != null && r.locationName!.isNotEmpty)
                            Padding(
                              ...
                              child: Row(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  const Icon(Icons.location_on, size: 12, color: kSub),
                                  const SizedBox(width: 4),
                                  Expanded(
                                    child: Text(
                                      r.locationName!,
                                      style: const TextStyle(fontSize: 11, color: kSub, height: 1.2),
                                      maxLines: 2,
                                      overflow: TextOverflow.ellipsis,
                                    ),
                                  ),
                ...
                SizedBox(
                  ...
                  child: TextButton.icon(
                    ...
                    label: const Text(
                      '삭제',
                      style: TextStyle(color: kRed, fontSize: 12, fontWeight: FontWeight.bold),
                    ),
                  ),
                ),
              ]),
